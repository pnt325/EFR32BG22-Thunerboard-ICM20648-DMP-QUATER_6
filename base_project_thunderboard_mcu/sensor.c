...
void inv_icm20648_enable_dmp_quater_6()
{
	int rc;
	// Enable sensor: INV_ICM20648_SENSOR_GAME_ROTATION_VECTOR
	INV_MSG(INV_MSG_LEVEL_VERBOSE, "Enable sensor: INV_ICM20648_SENSOR_GAME_ROTATION_VECTOR");
	rc = inv_icm20648_enable_sensor(&icm_device, INV_ICM20648_SENSOR_GAME_ROTATION_VECTOR, 1);
	check_rc(rc, "inv_icm20648_enable_sensor: error");
	enabled_sensor_mask |= (1 << INV_ICM20648_SENSOR_GAME_ROTATION_VECTOR);

	// Set update period
	rc = inv_icm20648_set_sensor_period(&icm_device, INV_ICM20648_SENSOR_GAME_ROTATION_VECTOR, 50);
	check_rc(rc, "inv_icm20648_set_sensor_period: error");

	INV_MSG(0, "Enable INV_ICM20648_SENSOR_GAME_ROTATION_VECTOR: DMP_QUATER6");
}
...
void sensor_event(const inv_sensor_event_t * event, void * arg){
	...
	*/
	...
	// TODO Handle quaternion data

//	INV_MSG(0, "Received data =====================");
//	INV_MSG(0, "Q0: %12u, Q1: %12u, Q2: %12u, Q3: %12u",
//			async_edata.d.async.sensorEvent.vdata.data.u32[0],
//			async_edata.d.async.sensorEvent.vdata.data.u32[1],
//			async_edata.d.async.sensorEvent.vdata.data.u32[2],
//			async_edata.d.async.sensorEvent.vdata.data.u32[3]);
    ...
    snprintf(buf, sizeof(buf), "x: %0.2f, y: %0.2f, z: %0.2f", roll, pitch, yaw);
    INV_MSG(0, "%s", buf);

    // Send data to uart
    memset(buf, 0, sizeof(buf));
    snprintf(buf, sizeof(buf), "$,%0.2f,%0.2f,%0.2f\r\n", roll, pitch, yaw);
    UARTDRV_Transmit(sl_uartdrv_usart_vcom_handle, (uint8_t*)buf, strlen(buf), NULL);
