...
/** @defgroup DriverIcm20648Transport Icm20648 driver transport
 *  @brief Low-level ICM20648 register access
 *  @ingroup  DriverIcm20648
 *  @{
 */

#ifndef _INV_ICM20648_TRANSPORT_H_
#define _INV_ICM20648_TRANSPORT_H_
...
void INV_EXPORT inv_icm20648_transport_init(struct inv_icm20648 * s);

int INV_EXPORT inv_icm20648_read_reg(struct inv_icm20648 * s, uint8_t reg, uint8_t * buf, uint32_t len);

int INV_EXPORT inv_icm20648_write_reg(struct inv_icm20648 * s, uint8_t reg, const uint8_t * buf, uint32_t len);

void INV_EXPORT inv_icm20648_sleep_100us(unsigned long nHowMany100MicroSecondsToSleep);

long INV_EXPORT inv_icm20648_get_tick_count(void);

static inline int inv_icm20648_write_reg_one(struct inv_icm20648 * s, uint8_t reg, uint8_t reg_value)
{
...
static inline int inv_icm20648_set_reg_bits(struct inv_icm20648 * s, uint8_t reg, uint8_t bits_mask)
{
	int rc;
	uint8_t reg_value;

	if((rc = inv_icm20648_read_reg_one(s, reg, &reg_value)) != 0)
		return rc;

	reg_value |= bits_mask;

	if((rc = inv_icm20648_write_reg_one(s, reg, reg_value)) != 0)
		return rc;

	return 0;
}

static inline int inv_icm20648_clear_reg_bits(struct inv_icm20648 * s, uint8_t reg, uint8_t bits_mask)
{
	int rc;
	uint8_t reg_value;
...
static inline int inv_icm20648_get_reg_bits(struct inv_icm20648 * s, uint8_t reg,
		uint8_t bits_mask, uint8_t * bits_mask_state)
{
	int rc;

	if((rc = inv_icm20648_read_reg_one(s, reg, bits_mask_state)) != 0)
		return rc;
...
#endif /* _INV_ICM20648_TRANSPORT_H_ */
