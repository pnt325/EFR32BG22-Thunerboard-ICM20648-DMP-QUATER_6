...
#include <ICM20648/Invn/Devices/Drivers/Icm20648/Icm20648DataBaseDriver.h>
...
#include <ICM20648/Invn/Devices/Drivers/Icm20648/Icm20648MPUFifoControl.h>
#include <ICM20648/Invn/Devices/Drivers/Icm20648/Icm20648Setup.h>
#include <ICM20648/Invn/Devices/SensorTypes.h>
#include <ICM20648/Invn/EmbUtils/dataconverter.h>
#include <ICM20648/Invn/EmbUtils/Message.h>
...
int inv_icm20648_init_scale(struct inv_icm20648 * s)
{
	/* Force accelero fullscale to 4g and gyr to 200dps */
	inv_icm20648_set_accel_fullscale(s, MPU_FS_4G);
	inv_icm20648_set_gyro_fullscale(s, MPU_FS_2000dps);
...
int inv_icm20648_get_fsr(struct inv_icm20648 * s, enum inv_icm20648_sensor sensor, const void * fsr)
{

	if((sensor == INV_ICM20648_SENSOR_RAW_ACCELEROMETER) ||
	   (sensor == INV_ICM20648_SENSOR_ACCELEROMETER)){
		unsigned char * castedvalue = (unsigned char*) fsr;
		int afsr = inv_icm20648_get_accel_fullscale(s);
		if(afsr == MPU_FS_2G)
			* castedvalue = 2;
		...
		int gfsr = inv_icm20648_get_gyro_fullscale(s);
...
int inv_icm20648_set_bias(struct inv_icm20648 * s, enum inv_icm20648_sensor sensor, const void * bias)
{
	int bias_q16[3];
	int bias_in[3];
	int rc = 0;
	short shift;
	switch(sensor) {
		case INV_ICM20648_SENSOR_ACCELEROMETER :
			...
			rc |= inv_icm20648_ctrl_set_acc_bias(s, bias_in);
			break;
		case INV_ICM20648_SENSOR_GYROSCOPE_UNCALIBRATED:
		case INV_ICM20648_SENSOR_GYROSCOPE:
			memcpy(bias_q16, bias, sizeof(bias_q16));
			//convert from q16 to :
			//Q19 => 2000dps
			//Q20 => 1000dps
			//Q21 => 500dps
			//Q22 => 250dps
			shift = ((20 + (MPU_FS_1000dps - inv_icm20648_get_gyro_fullscale(s))) - 16);
			bias_in[0] = bias_q16[0] << shift;
			bias_in[1] = bias_q16[1] << shift;
			bias_in[2] = bias_q16[2] << shift;

			rc |= inv_icm20648_ctrl_set_gyr_bias(s, bias_in);
			break;
		case INV_ICM20648_SENSOR_MAGNETIC_FIELD_UNCALIBRATED:
		case INV_ICM20648_SENSOR_GEOMAGNETIC_FIELD:
			memcpy(bias_q16, bias, sizeof(bias_q16));
			// bias is already in q16
			rc |= inv_icm20648_ctrl_set_mag_bias(s, bias_q16);
			break;
		default :
			rc = -1;
			break;
...
int inv_icm20648_get_bias(struct inv_icm20648 * s, enum inv_icm20648_sensor sensor, void * bias)
{
	int bias_qx[3];
	int bias_out[3];
	int rc = 0;
	...
	switch(sensor) {
		case INV_ICM20648_SENSOR_ACCELEROMETER :
			rc |= inv_icm20648_ctrl_get_acc_bias(s, bias_qx);
		...
		case INV_ICM20648_SENSOR_GYROSCOPE_UNCALIBRATED:
		case INV_ICM20648_SENSOR_GYROSCOPE:
			rc |= inv_icm20648_ctrl_get_gyr_bias(s, bias_qx);
		...
		case INV_ICM20648_SENSOR_MAGNETIC_FIELD_UNCALIBRATED:
		case INV_ICM20648_SENSOR_GEOMAGNETIC_FIELD:
			rc |= inv_icm20648_ctrl_get_mag_bias(s, bias_qx);
			// bias is already in q16
...
int inv_icm20648_get_lowpower_or_highperformance(struct inv_icm20648 * s, uint8_t * lowpower_or_highperformance)
{
	(void)s;
	*lowpower_or_highperformance = CHIP_LOW_NOISE_ICM20648;
	return 1;
}
...
int inv_icm20648_enable_batch_timeout(struct inv_icm20648 * s, unsigned short batchTimeoutMs)
{
	int rc;
	/* Configure batch timeout */
	if (inv_icm20648_ctrl_set_batch_timeout_ms(s, batchTimeoutMs) == 0) {
		/* If configuration was succesful then we enable it */
		if((rc = inv_icm20648_ctrl_enable_batch(s, 1)) != 0)
			return rc;
	} else {
		/* Else we disable it */
		if((rc = inv_icm20648_ctrl_enable_batch(s, 0)) != 0)
			return rc;
	}
	return 0;
}
