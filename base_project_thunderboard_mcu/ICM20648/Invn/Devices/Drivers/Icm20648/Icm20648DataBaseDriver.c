...
int inv_icm20648_get_proximity_availability(struct inv_icm20648 * s)
{
...
unsigned short inv_icm20648_get_secondary_divider(struct inv_icm20648 * s)
{
...
short inv_icm20648_get_accel_divider(struct inv_icm20648 * s)
{
...
*/
uint32_t inv_icm20648_get_odr_in_units(struct inv_icm20648 * s, unsigned short odrInDivider, unsigned char odr_units )
{
	unsigned long odr=0;
	unsigned long Us=0;
	...
	if(s->base_state.timebase_correction_pll == 0)
		inv_icm20648_read_mems_reg(s, REG_TIMEBASE_CORRECTION_PLL, 1, &s->base_state.timebase_correction_pll);
...
int inv_icm20648_set_serial_comm(struct inv_icm20648 * s, enum SMARTSENSOR_SERIAL_INTERFACE type)
{
	s->base_state.serial_interface = type;
...
int inv_icm20648_set_int1_assertion(struct inv_icm20648 * s, int enable)
{
	int   result = 0;
	...
	/*
	result         |= inv_icm20648_read_mems_reg(s, REG_INT_PIN_CFG, 1, &reg_pin_cfg);
	reg_pin_cfg    |= BIT_INT_LATCH_EN ;	// Latchen : BIT5 held the IT until register is read
	result         |= inv_icm20648_write_single_mems_reg(s, REG_INT_PIN_CFG, reg_pin_cfg);
	*/

	// Set int1 enable
	result |= inv_icm20648_read_mems_reg(s, REG_INT_ENABLE, 1, &reg_int_enable);
	...
	result |= inv_icm20648_write_single_mems_reg(s, REG_INT_ENABLE, reg_int_enable);
...
*  @return     0 if successful
*/
int inv_icm20648_accel_read_hw_reg_data(struct inv_icm20648 * s, short accel_hw_reg_data[3])
{
	int result = 0;
	uint8_t accel_data[6]; // Store 6 bytes for that

	// read mem regs
	result = inv_icm20648_read_mems_reg(s, REG_ACCEL_XOUT_H_SH, 6, (unsigned char *) &accel_data);
