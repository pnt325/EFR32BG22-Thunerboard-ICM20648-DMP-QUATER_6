...
/*
 *  inv_icm20648_write_akm_scale() - Configure the akm scale range.
 */
int inv_icm20648_write_akm_scale(struct inv_icm20648 * s, int data)
{
	char d, en;
	int result;
	...
	result = inv_icm20648_write_single_mems_reg(s, REG_I2C_SLV1_DO, d);
	if (result)
		return result;
...
/*
 *  inv_icm20648_read_akm_scale() - show AKM scale.
 */
int inv_icm20648_read_akm_scale(struct inv_icm20648 * s, int *scale)
{
#if (MEMS_CHIP == HW_ICM20948)
	(void)s;
...
char inv_icm20648_compass_getstate(struct inv_icm20648 * s)
{
	return s->secondary_state.secondary_resume_compass_state;
}

int inv_icm20648_compass_isconnected(struct inv_icm20648 * s)
{
	if(s->secondary_state.compass_state == INV_ICM20648_COMPASS_SETUP) {
		return 1;
	} else {
		return 0;
	}
