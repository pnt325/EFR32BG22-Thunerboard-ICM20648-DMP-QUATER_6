...
int inv_icm20648_identify_interrupt(struct inv_icm20648 * s, short *int_read)
{
    ...
    /*if(wake_on_motion_enabled) {
        result = inv_icm20648_read_mems_reg(s, REG_INT_STATUS, 1, &int_status);//INT_STATUS
        if(result)
            return result;
        *int_read |= reg_data[1];
    }*/
...
int inv_icm20648_dmp_process_fifo(struct inv_icm20648 * s, int *left_in_fifo, unsigned short *user_header, unsigned short *user_header2, long *time_stamp)
{
	...
	// no need to extract number of sample per sensor for current function, so provide 0 as last parameter
        ...
        //time stamp
        ts = inv_icm20648_get_tick_count();
