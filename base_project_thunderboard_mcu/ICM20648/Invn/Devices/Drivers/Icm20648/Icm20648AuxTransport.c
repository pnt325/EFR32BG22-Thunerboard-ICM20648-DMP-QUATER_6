...
* This is derived from inv_icm20648_read_secondary in linux...
...
/*
* inv_icm20648_write_secondary(): set secondary registers for writing?.
The chip must be set as bank 3 before calling.
* This is derived from inv_icm20648_write_secondary in linux...
* for now, uses a very simple data struct for the registers
* 
* index gives the mapping to the particular SLVx registers
* addr is the physical address of the device to be accessed
...
void inv_icm20648_secondary_saveI2cOdr(struct inv_icm20648 * s)
{
	inv_icm20648_read_mems_reg(s, REG_I2C_MST_ODR_CONFIG,1,&s->secondary_state.sSavedI2cOdr);
}

void inv_icm20648_secondary_restoreI2cOdr(struct inv_icm20648 * s)
{
